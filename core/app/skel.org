...
:phases
 { (:link-pod #<rule link-pod-core>)
   ...
   (:run #<rule run-swank>) ... }
:include
 [ #<project core/rust :components 1 :rules 3>
...
#+begin_example
:rules
 [ #<rule all (%stash psl.dat parquet.thrift rgb.txt build-zstd-alien
               build-tree-sitter-alien build-all-core)>
   ...
   #<rule mailman> #<rule alik>  #<rule rust-bin (mailman alik)>
   #<rule alik-ui> #<rule install (install-core)>  #<rule syms.sxp>
   #<rule dist>  #<rule dist-source (clean)>
   #<rule dist-core (clean %stash build-core)> ]
#+end_example
...
#+RESULTS:
#+begin_example
skel v0.1.1:15b26615e8d1 --- The hackable devtool.
