...
:END:
#+begin_src lisp :results replace :exports code
  ...
  (let ((g (make-instance 'directed-graph)))
    ...
    (sb-ext:run-program (cli:find-exe "dot") (list "-Tascii" "/tmp/bootstrap.dot" "-o" "/tmp/bootstrap.txt"))
    (with-open-file (f "/tmp/bootstrap.txt")
      (copy-stream f t)))
#+end_src
